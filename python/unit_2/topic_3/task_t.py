...
Формат вывода:
    Следует вывести номер первого блока, у которого неправильный хэш (не меньше 100 или не совпадает с вычисленным по указанной в условии формуле), или -1, если все хэши в блокчейне правильные.
...
for i in range(count):
    ...
    num_r = num // 256 % 256
    num_m = num // 256 ** 2
    current = (num_m + num_r + previous) * 37 % 256
    previous = current
    if wrong == -1 and (num_h != current or num_h >= 100):
        wrong = i
print(wrong)
